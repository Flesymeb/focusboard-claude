...
| --- | --- | --- |
| `loop-27-50abeca59b7b` | `warm_start:loop-26-50abeca59b7b` | PLANNED |
...
| Objective | Repair the candidate-owned first-run journey manifest into a fully protocol-native contract and fix the Calendar 390px overflow, so the host journey stage can independently verify PRD 3's release-critical path, PRD 9.2's network-error and narrow-Today states, and PRD 9's fits-its-parent rule — without altering pinned steps 1-7, the frozen CSP/capability set, or opt-in isolation gates, and without converting any host-owned receipt into a candidate claim. |
| Strategy | `strategy.protocol-native-manifest-repair-and-narrow-fix` |
| Tasks | 1 |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-26-50abeca59b7b` | `development_snapshot:attempt-8f7e28e6d99572805845d9ad` | BLOCKED |
...
| Status | PLANNED |
| Focus | phase / prd_implementation |
| Objective | Conform the journey manifest's status vocabulary to the product's PRD-correct DOM contract, add an explicitly opt-in isolated network-fault instrument and a reachable 390px narrow-viewport state with manifest phases for the PRD 9.2 recovery states, and keep the Linux URL-handler registration artifact bindable for the Host cold-activation probe, so the frozen host collector can execute and observe the complete first-run journey without any frozen assertion being weakened or normal startup behavior changing. |
| Strategy | `strategy.manifest-vocabulary-conformance-and-optin-instrumentation` |
| Tasks | 1 |
...
| Attempt | `attempt-8f7e28e6d99572805845d9ad` |
...
| --- | --- |
| Status | BLOCKED |
| Candidate | `loop-26-50abeca59b7b` |
...
| --- | --- | --- | --- | --- |
| `fnd.loop26.journey-drive-receipt-missing` | MAJOR | INTERNAL | HIGH | PRD 3 release-critical journey still has no independent host verdict: journey_drive is unmeasured (missing_receipt, repair_owner host); only the untrusted product self-drive ran (16/16 advisory vs 20 manifest steps). |
| `fnd.loop26.network-error-state-runtime-uncaptured` | MAJOR | INTERNAL | MEDIUM | PRD 9.2 network-error state is implemented and instrumented (gated fault + manifest phases) but still has no independent runtime capture; the only lane that can reach it (journey_drive) is missing. |
| `fnd.loop26.cold-activation-os-registration-unmeasured` | MAJOR | INTERNAL | HIGH | Cold focusboard:// activation and OS URL-handler registration remain unmeasured (external_activation missing_receipt); the host-probe registration script is now in place and startup-clean. |
| `fnd.loop26.calendar-narrow-390-overflow` | MINOR | DEGRADED | LOW | Observed product defect: Calendar at 390x844 overflows horizontally by 68px (month grid and header clip past the window edge) in two independent host captures, violating PRD 9's fits-its-parent rule. |
| `fnd.loop26.narrow-step-exceeds-launch-app-contract` | NOTE | INTERNAL | LOW | Manifest step narrow_today_390 requires relaunch-with-window semantics that the registered launch_app contract explicitly disclaims; the host journey stage must own this capability. |
| `fnd.loop26.isolated-store-flag-pass-and-scoped` | NOTE | INTERNAL | LOW | The isolated_native_store advisory flag now reports true/pass with an in-receipt scope note on both loop-26 receipts; the loop-24/25 unexplained-false concern no longer reproduces, supporting closure. |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-25-8df95e3b0742` | `development_snapshot:attempt-eac36904d9e42df7352da3a8` | BLOCKED |
...
| Changed paths | 6 |
| Blocker | See the QA outcome below. |
| Attempt | `attempt-eac36904d9e42df7352da3a8` |
...
| --- | --- |
| Status | BLOCKED |
| Candidate | `loop-25-8df95e3b0742` |
| Product completion | IN PROGRESS |
...
| ID | Severity | Impact | Release risk | Summary |
| --- | --- | --- | --- | --- |
| `fnd.loop25.journey-step8-status-vocabulary-mismatch` | MAJOR | INTERNAL | HIGH | Journey step 8 fails on an unsatisfiable pinned predicate: the manifest requires data-status=todo but the product only emits status values open/in_progress/completed, so the configure verdict can never pass even though the product behavior at that point is PRD-correct. |
| `fnd.loop25.golden-path-steps-9-18-unverified` | MAJOR | INTERNAL | HIGH | PRD 3 steps 9-18 have never executed in the host lane: focus cycle, exact-once completion, reminder delivery, sign-out/in persistence and all four recovery states still have no black-box runtime evidence, keeping the release-gate path unverified. |
| `fnd.loop25.prd92-network-error-and-390px-uncaptured` | MAJOR | INTERNAL | HIGH | PRD 9.2 network-error recovery state remains runtime-uncaptured (no WebDriver-reachable fault-injection hook) and narrow-viewport integrity is unmeasured because the 390px request is clamped to the 640px window minWidth, keeping a PRD 9.2/11 acceptance detail unverified. |
| `fnd.loop25.deeplink-cold-and-os-registration-untested` | MINOR | DEGRADED | MEDIUM | Warm focusboard:// activation is now host-verified, but cold activation and OS URL-handler registration remain untested, so out-of-box link activation on a fresh Linux install is still unproven (in-app paste affordance keeps the flow usable). |
| `fnd.loop25.isolated-store-advisory-flag-unexplained` | MINOR | INTERNAL | LOW | The isolated_native_store check again reports false as an unexplained advisory across receipts while the data-dir derivation unit test passes and the isolated-store probe is closed, so the flag's current meaning remains undocumented. |
...
| Status | PLANNED |
| Focus | golden_path |
| Objective | Add executable postconditions for every existing first-run and recovery journey step, preserving the complete frozen PRD and all independent Host acceptance requirements. |
| Strategy | `preserve-outcomes-measure-contract` |
| Tasks | 1 |
...
| Changed paths | 1 |
| Blocker | See the QA outcome below. |
| Attempt | `attempt-b2b99d694dfc602cfbeb62e8` |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-23-72467d3e1723` | `warm_start:loop-21-72467d3e1723` | DEVELOPER ATTEMPT INCOMPLETE |
...
| Status | PLANNED |
| Focus | golden_path |
| Objective | Make quality/first-run-journey.json an executable delivery contract: every journey step gains structured machine-checkable postconditions (expected_state.assertions, plus view_assertions for the multi-view step) expressed only with the validator's supported observation kinds, while every PRD section 3 step, recovery state, isolation rule, opt-in gate, and business outcome in the pinned manifest is preserved, so the host focusboard-native-webdriver journey stage can drive and grade the golden path and close gap:prd_golden_path. |
| Strategy | `executable-assertion-contract-rewrite` |
| Tasks | 1 |
...
| Changed paths | 0 |
| Blocker | See the QA outcome below. |
| Attempt | `attempt-20a7639d70efd70ad80a72fe` |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-22-72467d3e1723` | `warm_start:loop-21-72467d3e1723` | DEVELOPER ATTEMPT INCOMPLETE |
...
| Status | PLANNED |
| Focus | golden_path |
| Objective | Make the PRD section-3 first-run journey delivery contract executable by the registered replay lane through real visible UI only: retire the backend-held-credential self-drive evidence path, re-author quality/first-run-journey.json from advisory narrative into an atomic real-UI contract, and re-anchor the two evidence-stalled acceptances to lane-measurable forms with the structurally unavailable out-of-process deeplink receipt explicitly waived, so validate:prd_golden_path can pass on recapture. |
| Strategy | `golden-path-lane-executable-replacement` |
| Tasks | 1 |
...
| Changed paths | 0 |
| Blocker | See the QA outcome below. |
| Attempt | `attempt-66301fa6e66009487d571a81` |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-21-72467d3e1723` | `development_snapshot:attempt-c77d2319f0fe9b959c765311` | PASS |
...
| Status | PLANNED |
| Focus | final |
| Objective | Turn the PRD 9.2 recovery and responsive states into real, capturable product surfaces: a failed task mutation preserves the composed input, attributes the failure, and offers explicit Retry that reconciles to authoritative state; Today keeps its task hierarchy at narrow viewports without overlapping or off-screen controls; and Settings presents reminder timing, timezone, delivery state, and the test-email action compactly at 1280x800 instead of below the fold. All frozen baselines stay intact - the pinned journey manifest, the focusboard scheme registration, GoldenPathProbe opt-in isolation - and the loop-18 rewrite_acceptance records for the two pinned receipt issues are reaffirmed with fresh loop-21 verification. |
| Strategy | `prd92-states-first-rewrite-reaffirm` |
| Tasks | 1 |
...
| Changed paths | 12 |
| Blocker | See the QA outcome below. |
| Attempt | `attempt-c77d2319f0fe9b959c765311` |
...
| --- | --- |
| Status | PASS |
| Candidate | `loop-21-72467d3e1723` |
...
| ID | Severity | Impact | Release risk | Summary |
| --- | --- | --- | --- | --- |
| `fnd.loop21.prd92-states-one-of-three-captured` | MINOR | INTERNAL | LOW | PRD 9.2 progress on the loop-21 candidate: the Settings reminder-deliveries detail is now runtime-verified above the fold in both independent drives, and the composer-failure and narrow-viewport states are white-box implemented (opt-in fault, preserved input, single-shot Retry, 760px wrap rules), but the fault-injected network-error and narrow-viewport Today states remain runtime-uncaptured — the host collector has no fault or narrow stage and tauri-driver/WebKitWebDriver are unavailable to the developer and tester roles. |
| `fnd.loop21.deeplink-external-activation-carried` | MINOR | INTERNAL | MEDIUM | External focusboard:// warm/cold activation remains receipt-unproven on loop-21: external_deeplink=not_run in both receipts with the collector-lane reason; scheme registration re-verified present with no drift and the ledger stays unit-proven in rust-unit 50/50. Carried host-lane gap, unchanged in kind. |
| `fnd.loop21.section3-receipt-carried` | MINOR | INTERNAL | MEDIUM | No host-collected per-step PRD section-3 journey receipt exists on the loop-21 candidate: both receipts record evidence_scope=anonymous_smoke_plus_triggered_product_self_drive with independently_verified=false; the pinned journey manifest is byte-identical (independently re-hashed), the advisory drive passed 16/16 host-observed, and host-collected evidence covers the anonymous smoke plus signed-in navigation of all six surfaces across two drives. Carried for host follow-up. |
| `fnd.loop21.auth-recovery-journey-carried` | MINOR | INTERNAL | LOW | Forgot/reset/verify recovery views and the backend auth chain remain delivered and unit-proven (single-use/expired/reused token tests inside rust-unit 50/50), but no bound loop-21 capture drives forgot-password through email-link reset to sign-in in the real UI and no email-action page is captured. Carried evidence gap. |
| `fnd.loop21.freeze-baseline-holds` | NOTE | INTERNAL | LOW | Tracking note: freeze-baseline validations hold on loop-21 — the journey manifest is byte-identical (independently re-hashed to the pinned sha), rust-unit is 50/50 in the fresh receipt, both receipts bind the same candidate sha with source_unchanged and native_candidate_matches true, and the change manifest scopes the diff to the 9.2 increment plus its dist rebuild. |
| `fnd.loop21.inbox-change-reverified` | NOTE | INTERNAL | LOW | Verified again live on loop-21: the loop-20 Inbox filtered-empty change renders correctly in both independent host drives with wrapped struck-through completed titles, intact controls, and no regression on any other captured surface; white-box gates green. Closing as verified across two consecutive loops. |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-20-b49070167c09` | `development_snapshot:attempt-82dc9e783926305532771276` | PASS |
...
| Status | PLANNED |
| Focus | golden_path |
| Objective | Close gap:prd_golden_path by making the complete PRD section-3 first-run path — including the forgot/reset/verify recovery journey and deep-link activation observability — drivable end-to-end through the real UI under a strictly opt-in isolated profile, so the current host instruments can produce the previously missing per-step receipts; treat the three pinned evidence-stalled issues as one cluster and dispose of each through this task's measurement-infrastructure resolution. |
| Strategy | `receipt-chain-closure-via-candidate-observability` |
| Tasks | 1 |
...
| Changed paths | 4 |
| Blocker | See the QA outcome below. |
| Attempt | `attempt-82dc9e783926305532771276` |
...
| --- | --- |
| Status | PASS |
| Candidate | `loop-20-b49070167c09` |
| Product completion | IN PROGRESS |
| Review scope | `golden_path` |
| Findings | 6 (0 blocking focus items) |
...
| ID | Severity | Impact | Release risk | Summary |
| --- | --- | --- | --- | --- |
| `fnd.loop20.section3-journey-receipt-carried` | MINOR | INTERNAL | MEDIUM | No host-collected per-step PRD section-3 journey receipt exists on the loop-20 candidate: both receipts again record evidence_scope=anonymous_smoke_plus_triggered_product_self_drive with independently_verified=false; the triggered 16/16 report is renderer-self-drive-ipc and untrusted; host-collected evidence adds anonymous smoke plus signed-in navigation of all six surfaces across two independent drives. Carried for host follow-up per the risk handoff. |
| `fnd.loop20.deeplink-external-activation-carried` | MINOR | INTERNAL | MEDIUM | External focusboard:// warm/cold activation remains receipt-unproven on loop-20: external_deeplink=not_run in both receipts (collector launches the app directly); scheme registration and ledger handling stay delivered and unit-proven in rust-unit 50/50. Carried host-lane gap, unchanged in kind from loop 19. |
| `fnd.loop20.auth-recovery-journey-carried` | MINOR | INTERNAL | LOW | Forgot/reset/verify recovery views and the backend auth chain remain delivered and unit-proven (single-use/expired/reused token tests inside rust-unit 50/50), but no bound loop-20 capture drives forgot-password through email-link reset to sign-in in the real UI and no email-action page is captured. Carried evidence gap. |
| `fnd.loop20.prd92-states-carried` | MINOR | INTERNAL | LOW | PRD 9.2 recovery/responsive states remain runtime-uncaptured on loop-20: all 13 bound captures are 1280x800 success or signed-in states; no fault-injected network-error with preserved input, no narrow-viewport Today, and Settings REMINDER DELIVERIES detail again begins at the 800px fold in both fresh captures. Carried evidence gap. |
| `fnd.loop20.freeze-baseline-tracking` | NOTE | INTERNAL | LOW | Tracking note: freeze-baseline validations hold on loop-20 - the journey manifest is byte-identical across loop-19 and loop-20 (sha256 8842405b905fa993328452290915138992fc644fdacae1c40bbdbdec07d30dfe, independently re-hashed), rust-unit is 50/50 in the fresh receipt, and both receipts bind the same candidate sha with source_unchanged and native_candidate_matches true. |
| `fnd.loop20.inbox-filtered-empty-change-verified` | NOTE | INTERNAL | LOW | Verified live: the loop-20 change (Inbox filtered-empty state with Clear filters, active-filter result count, and the open/completed split) renders correctly in both independent host drives with wrapped struck-through completed titles, intact controls, and no regression on any other captured surface; all white-box gates stay green. |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-19-594829ca7661` | `development_snapshot:attempt-1f32073f190afa84bfa82d01` | PHASE PASS / PRODUCT IN PROGRESS |
...
| Status | PLANNED |
| Focus | phase / prd_implementation |
| Objective | Close the final candidate-owned defect by fixing the Inbox completed-task-row wrap as a presentation-only change that preserves the frozen Tauri golden-path baseline, and lock the rewritten instrument-bound acceptances for the deeplink and first-run-journey evidence issues so the phase gate can pass with external activation, per-step journey, and recovery-state captures left as named Host follow-ups. |
| Strategy | `presentation-closure-with-locked-acceptance-baseline` |
| Tasks | 1 |
...
| Changed paths | 6 |
| Blocker | See the QA outcome below. |
| Attempt | `attempt-1f32073f190afa84bfa82d01` |
...
| --- | --- |
| Status | PASS |
| Candidate | `loop-19-594829ca7661` |
| Product completion | IN PROGRESS |
| Review scope | `phase` |
| Findings | 6 (0 blocking focus items) |
...
| ID | Severity | Impact | Release risk | Summary |
| --- | --- | --- | --- | --- |
| `fnd.loop19.completed-row-wrap-fix-verified` | NOTE | DEGRADED | LOW | Verified fixed: completed task rows keep metadata controls on one line with the checkbox aligned at row start; the long title wraps within its own column, which PRD 9 permits. Confirmed visually in both loop-19 host runs (Inbox, Today, Projects) and structurally in the compiled CSS. |
| `fnd.loop19.deeplink-external-activation-carried` | MINOR | INTERNAL | MEDIUM | External focusboard:// warm/cold activation remains receipt-unproven: external_deeplink=not_run in both loop-19 receipts (collector launches the app directly); scheme registration, warm/cold handling, and the deep_link_status ledger stay delivered and unit-proven (focusboard_scheme and deeplink_ledger tests in rust-unit 50/50). |
| `fnd.loop19.section3-journey-receipt-carried` | MINOR | INTERNAL | MEDIUM | No host-collected per-step PRD section-3 journey receipt exists: loop-19 receipts again contain anonymous smoke, the triggered 16/16 advisory self-drive (untrusted), and host-collected signed-in surface navigation only. |
| `fnd.loop19.auth-recovery-journey-carried` | MINOR | INTERNAL | LOW | Forgot/reset/verify recovery views and the backend auth chain remain delivered and unit-proven (single-use/expired/reused token tests pass in rust-unit 50/50), but no bound capture drives forgot -> email-link reset -> sign-in through the real UI and no email-action page is captured. |
| `fnd.loop19.prd92-states-carried` | MINOR | INTERNAL | LOW | PRD 9.2 recovery/responsive states remain runtime-uncaptured: all 13 loop-19 bound captures are 1280x800 success or signed-in states; no fault-injected network-error with preserved input, no narrow-viewport Today, and Settings REMINDER DELIVERIES detail begins at the 800px capture fold. |
| `fnd.loop19.freeze-baseline-tracking` | NOTE | INTERNAL | LOW | Tracking note: freeze-baseline validations hold this loop - the journey manifest stays pinned byte-unchanged, rust-unit is 50/50 including the golden-path manifest tests, and the rewritten acceptances for the three carried host issues remain the routing truth. |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-18-cd6ff4ca9b29` | `development_snapshot:attempt-0e3e1714f87a3dfbc27a409e` | PASS |
...
| Status | PLANNED |
| Focus | final |
| Objective | Deliver a frozen, verified loop-18 candidate whose three evidence-stalled issues carry formally rewritten, candidate-measurable acceptances recorded in quality/evidence-acceptances.md, with the pinned journey bridge, deep-link scheme registration, recovery UI, security posture, and 50/50 unit suite preserved unchanged, so the Host can register its consuming stages and the final prd_product_review can accept the complete PRD surface without another candidate-side measurement patch. |
| Strategy | `acceptance-rewrite-and-host-followup-isolation` |
| Tasks | 1 |
...
| Changed paths | 1 |
| Blocker | See the QA outcome below. |
| Attempt | `attempt-0e3e1714f87a3dfbc27a409e` |
...
| --- | --- |
| Status | PASS |
| Candidate | `loop-18-cd6ff4ca9b29` |
| Product completion | PASS |
| Review scope | `product` |
| Findings | 6 (0 blocking focus items) |
...
| ID | Severity | Impact | Release risk | Summary |
| --- | --- | --- | --- | --- |
| `fnd.loop18.freeze-baseline-validated` | NOTE | INTERNAL | LOW | Risk-handoff validated: rewritten acceptances are recorded for all three pinned issues, the pinned journey manifest is byte-unchanged (sha 8842405b), and the host white-box receipt shows rust-unit 50/50 including the golden_path.rs manifest tests. |
| `fnd.loop18.deeplink-external-activation-carried` | MINOR | INTERNAL | MEDIUM | External focusboard:// warm/cold activation remains receipt-unproven (external_deeplink=not_run in both loop-18 receipts, collector-lane reason); scheme registration, warm/cold handling, and the deep_link_status ledger are delivered and unit-proven; carried as a named host follow-up. |
| `fnd.loop18.section3-external-journey-carried` | MINOR | INTERNAL | LOW | No host-collected per-step PRD section-3 journey receipt exists: both loop-18 receipts contain anonymous smoke, the triggered 16/16 advisory self-drive (explicitly untrusted), and host-collected signed-in surface navigation; the drivable journey manifest is pinned and its validation tests pass; carried as a named host follow-up. |
| `fnd.loop18.auth-recovery-journey-carried` | MINOR | INTERNAL | LOW | Forgot/reset/verify recovery views and the backend auth chain are delivered and unit-proven, but no bound capture drives forgot -> email-link reset -> sign-in through the real UI and no email-action page is captured; carried as a host collector follow-up per the rewritten acceptance. |
| `fnd.loop18.prd92-states-carried` | MINOR | INTERNAL | LOW | PRD 9.2 recovery/responsive states remain runtime-uncaptured: all 13 bound captures across two runs are 1280x800 success or signed-in states; no fault-injected network-error with preserved input, no narrow-viewport Today, and the Settings REMINDER DELIVERIES detail begins below the 800px capture fold; carried as a host collector follow-up. |
| `fnd.loop18.completed-row-wrap-polish` | MINOR | DEGRADED | LOW | Polish note: on the Inbox surface a completed task row wraps its metadata controls across two lines with the checkbox dropping below the strikethrough title; all text fits its parent and nothing overlaps, so this is alignment polish only, not a PRD violation. |
...
| Candidate | Base | Current state |
| --- | --- | --- |
| `loop-17-cc5832463442` | `development_snapshot:attempt-51e2868241f1594e3fa28136` | PASS |
...
| --- | --- |
| Status | PASS |
| Candidate | `loop-17-cc5832463442` |
| Product completion | IN PROGRESS |
| Review scope | `golden_path` |
...
| ID | Severity | Impact | Release risk | Summary |
| --- | --- | --- | --- | --- |
| `fnd.loop17.section3-external-journey-receipt-still-absent` | MINOR | INTERNAL | HIGH | Risk-handoff validated: the developer's journey-measurement bridge is delivered and candidate-bound (quality/first-run-journey.json sha 8842405b… matches the change manifest; golden_path.rs:288-390 manifest tests pass within rust-unit 50/50), but no manifest-consuming host stage ran — both loop-17 receipts again contain only anonymous smoke, the host-triggered 16/16 advisory self-drive (untrusted), and post-drive surface navigation; external_deeplink stays not_run. The external section-3 journey receipt remains unproduced: explicit host-lane blocked disposition backed by the delivered bridge, not an unmeasured carry. Severity re-scoped from the ledger's major because no product failure is observed and the remaining step is a single mechanical host registration. |
| `fnd.loop17.deeplink-external-activation-lane-absent` | NOTE | INTERNAL | LOW | External focusboard:// activation remains receipt-unproven: external_deeplink is not_run in both loop-17 receipts with the collector-lane reason (no out-of-process activation stage). Product-side handling stays unit-proven within the 50/50 suite. Blocked lane, tracked; no product verdict possible from this environment. |
| `fnd.loop17.advisory-whitebox-lanes-calibrated` | NOTE | INTERNAL | LOW | Verified fixed: both advisory white-box lanes now emit 'superseded' with the calibrated reason naming the modular layout and the authoritative tauri_verify lane, instead of blocking on the absent src-tauri/src/commands.rs or claiming semantic 1/5 against the live implementation; the same receipt shows tsc/vite/fmt green and 50/50 rust tests. Instrument-vs-product misattribution has stopped; find.advisory-whitebox-stale-layout closes. Residual cosmetic: the advisory_white_box summary map still labels the superseded lanes 'fail' at exit-code level. |
| `fnd.loop17.auth-recovery-journey-still-uncaptured` | MINOR | INTERNAL | MEDIUM | Forgot/reset password flows remain journey-uncaptured on loop-17 evidence: the bound anonymous captures show only the Forgot password? link; no receipt drives forgot → email-link reset → sign-in through the real UI and no email action page is captured. Backend chain stays unit-proven in the 50/50 suite. Evidence debt, not an observed defect. |
| `fnd.loop17.prd92-recovery-responsive-states-still-uncaptured` | MINOR | INTERNAL | MEDIUM | PRD 9.2 recovery/responsive states remain runtime-uncaptured: all 15 loop-17 bound captures across two independent runs are 1280x800 success or anonymous-auth states. No fault-injected network-error retry with preserved input, no narrow-viewport Today, and the Settings REMINDER DELIVERIES detail falls below the capture fold. Evidence debt against the PRD 9.2 quality bar, not an observed product failure. |
...
- Latest candidate: `loop-27-50abeca59b7b`
- Accepted candidate: `loop-18-cd6ff4ca9b29`
- Latest attempted: `loop-27-50abeca59b7b`
- Latest warm start: `loop-27-50abeca59b7b`
- Last published: `loop-26-50abeca59b7b`
- Best verified: `loop-26-50abeca59b7b`
